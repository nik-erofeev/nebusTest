...
def create_tokens(data: dict) -> dict:
    ...
    access_token = jwt.encode(access_payload, APP_CONFIG.SECRET_KEY, algorithm=APP_CONFIG.ALGORITHM)

    # RefreshToken - 7 дней
    refresh_expire = now + timedelta(days=7)
    refresh_payload = data.copy()
    refresh_payload.update({"exp": int(refresh_expire.timestamp()), "type": "refresh"})
    refresh_token = jwt.encode(refresh_payload, APP_CONFIG.SECRET_KEY, algorithm=APP_CONFIG.ALGORITHM)
